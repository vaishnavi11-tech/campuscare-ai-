...
function ManageStaff() {
  ...
  const handleSubExpertiseToggle = (item, checked) => {
    if (checked) {
      setFormData({
        ...formData,
        subExpertise: [...formData.subExpertise, item],
      });
    } else {
      setFormData({
        ...formData,
        subExpertise: formData.subExpertise.filter((x) => x !== item),
      });
    }
  };
  ...
  return (
    <Layout>
      <div className="max-w-6xl mx-auto p-8">
        ...
        <p className="text-gray-500 mb-8">Create and manage faculty accounts.</p>
          ...
          <form onSubmit={handleCreateStaff} className="grid md:grid-cols-2 gap-4">
            ...
            {SUB_EXPERTISE[formData.expertise] && (
              <div className="md:col-span-2 border rounded-lg p-3">
                <p className="font-medium mb-2">Select Sub Expertise</p>

                <div className="grid grid-cols-2 gap-2">
                  {SUB_EXPERTISE[formData.expertise].map((item) => (
                    <label key={item} className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={formData.subExpertise.includes(item)}
                        onChange={(e) =>
                          handleSubExpertiseToggle(item, e.target.checked)
                        }
                      />
                      {item}
                    </label>
            ...
            {formData.expertise === "Hostel & Accommodation" && (
              <select
                value={formData.hostelWing}
                onChange={(e) =>
                  setFormData({ ...formData, hostelWing: e.target.value })
                }
                className="border p-3 rounded-lg"
                required
              >
            ...
            >
              <div>
                ...
                {member.subExpertise?.length > 0 && (
                  <p className="text-sm text-gray-500">
                    Sub Expertise: {member.subExpertise.join(", ")}
                  </p>
                )}
                {member.department && (
                  <p className="text-sm text-gray-500">
